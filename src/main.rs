...
use primitives::{KeyType, NotMut, SendBox, XKeyClicker, KeyBehavior};
...
fn on_start(xkc_handle: &ArcXKeyClicker) {
    // Apply start delay
    let start_delay = *xkc_handle.start_delay.lock().unwrap();
    if start_delay > 0 {
        sleep(std::time::Duration::from_secs(start_delay));
    }
...
fn build_ui(app: &Application, entry_sender: Sender<Entry>, xkc_handle: ArcXKeyClicker) {
    ...
    start_delay_entry.connect_changed(move |entry| {
        if let Ok(delay) = entry.buffer().text().parse::<u64>() {
    ...
    repeat_count_entry.connect_changed(move |entry| {
        if let Ok(count) = entry.buffer().text().parse::<u64>() {
    ...
    let entry_sender_for_add = entry_sender_copy.clone();
    let xkc_handle_for_add = xkc_handle.clone();
    add_key_button.connect_clicked(move |_| {
        set_keybind(
            &entry_sender_for_add.clone(),
            &key_status_entry,
            &xkc_handle_for_add.clone(),
            KeyType::AddKey,
        );
    });
...
fn keybind(event: &Event, receiver: &Arc<SendBox<Receiver<Entry>>>, xkc_handle: &ArcXKeyClicker) {
    ...
    } = event
    {
        ...
        if let KeyType::AddKey = *should_recv {
            xkc_handle.add_key_action(*key);
            *should_recv = KeyType::None;

            if let Ok(entry) = receiver.0.try_recv() {
